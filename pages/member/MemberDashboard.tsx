...
export const MemberDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto animate-fade-in">
      ...
      {/* Grid Principal Responsivo: 1 coluna no mobile, 12 colunas no desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Coluna Principal (Esquerda): Versículo, Dízimo/PIX, Atalhos e Oração */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {quickActions.map(({ label, icon: Icon, color, action }) => (
                <button
                  key={label}
                  onClick={action}
                  className="flex flex-col items-center gap-2 p-3 bg-white hover:bg-orange-50/50 rounded-2xl border border-gray-100 hover:border-orange-200 shadow-sm hover:shadow transition-all active:scale-95"
                >
        ...
        {/* Coluna Secundária (Direita): Atividade Recente e Próximos Eventos */}
