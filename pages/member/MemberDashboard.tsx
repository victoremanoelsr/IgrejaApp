  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-100">{firstNameFormatted} 👋</h1>
      </div>

      {/* Tithes Card */}
      <div className="relative bg-gradient-to-br from-orange-500 to-orange-700 rounded-xl p-5 shadow-xl shadow-orange-900/30 overflow-hidden">
        ...
        <div className="relative">
          ...
          {church.pixKey && (
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 bg-white/15 hover:bg-white/25 border border-white/20 rounded-lg px-4 py-2 text-white text-xs font-semibold transition-all active:scale-95"
            >
              {copied ? (
                <>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-gray-100 font-semibold text-sm truncate">{member.name}</p>
            <p className="text-gray-400 text-xs">{church.name}</p>
            {member.memberNumber && (
              <p className="text-gray-500 text-xs">Nº {member.memberNumber}</p>
            )}
          </div>
          <span
          ...
          <div className="space-y-2">
            {upcomingEvents.map((event) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-gray-100 text-xs font-semibold truncate">{event.name}</p>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-[10px]">{formatDate(txn.date)}</p>
                  </div>
                </div>
