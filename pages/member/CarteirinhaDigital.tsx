...
export const CarteirinhaDigital: React.FC = () => {
  ...
  const qrData = encodeURIComponent(`IgrejaApp|${member.id}|${member.cpf}|${church.name}`);
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${qrData}&bgcolor=0f172a&color=f97316&qzone=1`;

  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({ title: 'Carteirinha Digital', text: `${member.name} — ${church.name}` });
    }
  };
  ...
  return (
    <div className="space-y-6 w-full max-w-5xl mx-auto animate-fade-in">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Seu documento de membro oficial</p>
      </div>

      {/* Grid Responsivo: 1 coluna no mobile, 12 colunas no desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Coluna Esquerda: O Cartão Digital e Ações */}
        <div className="lg:col-span-7 space-y-4">
          {/* Card */}
          <div ref={cardRef} className="relative rounded-2xl overflow-hidden shadow-xl border border-slate-700/50">
            ...
            <div className="relative p-6">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2.5">
                  {church.logoUrl ? (
                    <img src={church.logoUrl} alt="Logo" className="w-9 h-9 rounded-full object-cover border-2 border-white/20" />
                  ) : (
                  ...
                  <div>
                    ...
                    <p className="text-orange-400 text-[10px] font-bold uppercase tracking-widest">Membro Oficial</p>
                  </div>
                </div>
                <div className="flex items-center gap-1 bg-emerald-500/15 border border-emerald-500/30 rounded-full px-2.5 py-1">
                  <Shield size={11} className="text-emerald-400" />
                  <span className="text-emerald-400 text-[10px] font-bold">{member.status || 'ATIVO'}</span>
                </div>
              </div>
              ...
              <div className="flex items-end justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {member.memberNumber && (
                      <p className="text-orange-400 text-xs font-bold mt-1">Nº {member.memberNumber}</p>
                    )}
                    {member.baptismDate && (
                      <p className="text-slate-400 text-[11px] mt-0.5">Batismo: {formatDate(member.baptismDate)}</p>
                    )}
                  </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-1.5 shrink-0">
                  <img src={qrUrl} alt="QR Code" width={80} height={80} className="rounded-lg" loading="lazy" />
                </div>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-300 text-xs font-semibold tracking-wider">
                    {member.cpf ? member.cpf.replace(/\D/g, '').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4') : '—'}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-slate-500 text-[9px] font-bold uppercase tracking-wider">Emitido por</p>
          ...
          {typeof navigator.share !== 'undefined' && (
            <button onClick={handleShare}
              className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 active:scale-[0.98] text-white font-semibold py-3 rounded-xl transition-all shadow-sm">
              <Download size={16} />
              Compartilhar Carteirinha
            </button>
          )}

          <p className="text-center text-gray-400 text-[11px] pb-2">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-gray-400 text-[10px] font-semibold uppercase tracking-wider">Igreja</p>
              <p className="text-gray-800 text-sm font-semibold truncate">{church.name}</p>
              {church.pastorName && <p className="text-gray-400 text-xs truncate">Pastor: {church.pastorName}</p>}
            </div>
          </div>
        </div>
      </div>
