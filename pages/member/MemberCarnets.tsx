...
export const MemberCarnets: React.FC = () => {
  ...
  const getMonthNameForLang = (monthIndex: number) =>
  ...
  const handleGeneratePDF = async (templateId: string) => {
    ...
    try {
      ...
      const replacements: Record<string, string> = {
        ...
        '{{cpf}}': member.cpf,
        '{{mes_extenso}}': getMonthNameForLang(now.getMonth()),
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{t('memberPortal.carnets.subtitle')}</p>
      </div>
            ...
            <div key={template.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  ...
                  {t('memberPortal.carnets.downloadPdf')}
                </button>
              </div>
            </div>
          ))}
      ...
      <p className="text-center text-gray-400 text-xs pb-2">
        {t('memberPortal.carnets.pdfAutoGenerated')}
      </p>
