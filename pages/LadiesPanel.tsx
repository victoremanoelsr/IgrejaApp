...
const LADIES_ROLES: {role: Role, label: string}[] = [
    ...
    { role: 'TESOUREIRO_SENHORAS', label: 'Tesoureiro(a) de Senhoras' }
];

export const LadiesPanel: React.FC = () => {
  const { 
  ...
  const [teamFormData, setTeamFormData] = useState({ name: '', username: '', password: '', role: 'LIDER_SENHORAS' as Role, cpf: '' });
  ...
  const historyTransactions = transactions.filter(t => {
      ...
      if (t.category !== 'SENHORAS') return false; 
  ...
  const handleTransactionSubmit = async (e: React.FormEvent) => {
      ...
      if (transType === 'SAIDA' && isRecurring) {
          ...
          fixedId = await addFixedExpense({
              ...
              dueDay: day, category: 'SENHORAS', autoGenerate: true, active: true
          });
      }

      await addTransaction({
          id: '', churchId: currentChurch.id, type: transType, category: 'SENHORAS', amount: parseFloat(amount),
  ...
  const toggleLadyMember = async (member: Member) => {
      await updateMember(member.id, { ...member, isLady: !member.isLady });
      showFeedback(member.isLady ? 'Removida do Círculo de Oração' : 'Adicionada ao Círculo de Oração');
  };
  ...
  const renderDashboard = () => {
      ...
      // CÁLCULO TOTAL DE SENHORAS
      const totalLadies = members.filter(m => m.churchId === currentChurch?.id && m.isLady).length;
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-12 gap-6">
              ...
              <div onClick={() => setActiveTab('CAIXA')} className="col-span-12 md:col-span-8 bg-white rounded-xl shadow-sm border-l-4 border-pink-500 p-6 flex flex-col justify-center relative overflow-hidden cursor-pointer hover:-translate-y-1 hover:shadow-lg transition-all duration-200 group">
  ...
  const renderMembersTab = () => {
      const ladyMembers = members.filter(m => m.churchId === currentChurch?.id && m.isLady);
      ...
      return (
          <div className="bg-white p-6 rounded-xl shadow border">
              <h3 className="font-bold text-gray-700 mb-4 flex items-center"><Heart className="mr-2"/> Círculo de Oração</h3>
              
              <div className="mb-6">
                  <label className="block text-xs font-bold text-gray-500 mb-1">Adicionar / Buscar Irmã</label>
                      ...
                      <div className="mt-2 max-h-40 overflow-y-auto border rounded bg-gray-50">
                          {allMembers.slice(0, 10).map(m => (
                              <div key={m.id} className="p-2 border-b flex justify-between items-center hover:bg-white">
                                  ...
                                  <button 
                                      onClick={() => toggleLadyMember(m)}
                                      className={`px-2 py-1 rounded text-[10px] font-bold ${m.isLady ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}
                                  >
                                      {m.isLady ? 'Remover' : 'Adicionar'}
                                  </button>
                              </div>
                          ))}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {ladyMembers.map(m => (
                      <div key={m.id} className="p-3 border rounded-lg flex items-center gap-3 bg-gray-50">
                          <div className="h-10 w-10 bg-pink-100 rounded-full flex items-center justify-center text-pink-600 font-bold text-xs">
                              {m.name.charAt(0)}
                          </div>
                          ...
                          <button onClick={() => toggleLadyMember(m)} className="text-gray-400 hover:text-red-500"><X size={16}/></button>
                      </div>
                  ))}
                  {ladyMembers.length === 0 && <p className="col-span-full text-center text-gray-400 py-4">Nenhuma irmã cadastrada.</p>}
              </div>
          </div>
      );
  };
              ...
              <form onSubmit={handleSaveTeamMember} className="space-y-4 max-w-lg mx-auto bg-gray-50 p-6 rounded-xl border">
                  ...
                  <div className="relative" ref={wrapperRef}>
                      ...
                      <input 
                        required 
                        autoComplete="off"
                        className="w-full p-2.5 border rounded-lg uppercase text-sm focus:ring-2 focus:ring-pink-500 outline-none" 
                        ...
                        <div className="absolute z-10 w-full bg-white mt-1 border border-gray-200 rounded-lg shadow-xl max-h-40 overflow-y-auto">
                          {memberSuggestions.map((member) => (
                            ...
                            >
                              ...
                              <PlusCircle size={14} className="text-pink-600" />
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  <div>
                      ...
                      <select className="w-full p-2.5 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-pink-500 outline-none" value={teamFormData.role} onChange={e => setTeamFormData({...teamFormData, role: e.target.value as Role})}>
                  ...
                  <div className="flex gap-3 pt-4">
                      ...
                      <button type="submit" className="flex-1 py-2.5 bg-pink-600 text-white rounded-lg text-sm font-bold hover:bg-pink-700 transition-colors shadow-md">Salvar Membro</button>
                  </div>
              </form>
          )}
      </div>
  ...
  const renderReports = () => {
      ...
      const generatePDFReport = () => {
        ...
        doc.setFontSize(18);
        doc.text("Relatório de Senhoras", 14, 20);
        doc.setFontSize(10);
        ...
        doc.save(`Relatorio_Senhoras_${periodText.replace(/\//g, '-')}.pdf`);
    };
                  ...
                  {reportViewMode === 'DETAILED' ? (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          <div>
                              ...
                              <div className="space-y-4">
                                  <div className="border rounded-lg overflow-hidden">
                                      <div className="bg-green-50 p-3 flex justify-between items-center border-b border-green-100"><span className="font-bold text-green-800 text-sm">Entradas de Senhoras</span><span className="text-xs font-bold text-green-600">Total: {formatCurrency(totalIn)}</span></div>
                          ...
                          <div>
                              ...
                              <div className="space-y-4">
                                  <div className="border rounded-lg overflow-hidden">
                                      <div className="bg-red-50 p-3 flex justify-between items-center border-b border-red-100"><span className="font-bold text-red-800 text-sm">Saídas de Senhoras</span><span className="text-xs font-bold text-red-600">Total: {formatCurrency(totalOut)}</span></div>
                                      <div className="bg-white">
                            ...
                            <div className="bg-white rounded-lg shadow overflow-hidden border border-gray-100">
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    {historyTransactions.map(t => {
                                        ...
                                        <td className="hidden md:table-cell px-2 py-2 text-xs"><span className={`px-1 rounded text-[10px] font-bold ${t.type === 'ENTRADA' ? 'bg-pink-100 text-pink-700' : 'bg-red-100 text-red-700'}`}>{t.category}</span></td>
