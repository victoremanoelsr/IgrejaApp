...
export const ChildrenPanel: React.FC = () => {
  ...
  const canAddToTeam = user && ['SUPER_ADM', 'PRESIDENTE', 'VICE_PRESIDENTE', 'DIRIGENTE', 'SECRETARIO', 'LIDER_CRIANCAS'].includes(user.role);
  ...
  const handleSaveTeamMember = async (e: React.FormEvent) => {
      ...
      if (editingUserId) {
          ...
          if (existingUser) {
              const otherRoles = getUserRoles(existingUser).filter(r => !CHILDREN_ROLES.some(cr => cr.role === r));
          ...
          if (existingUser) {
              ...
              showFeedback('Usuário já existente: cargo infantil adicionado!');
          } else {
              const payload: User = {
                  id: '',
                  name: teamFormData.name.toUpperCase(),
  ...
  const renderDashboard = () => {
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-12 gap-6">
              ...
              <div onClick={() => setActiveTab('CAIXA')} className="col-span-12 md:col-span-8 bg-white rounded-xl shadow-sm border-l-4 border-blue-500 p-6 flex flex-col justify-center relative overflow-hidden cursor-pointer hover:-translate-y-1 hover:shadow-lg transition-all duration-200 group">
