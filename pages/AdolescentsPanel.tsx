  ...
  const renderDashboard = () => {
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-12 gap-6">
              ...
              <div onClick={() => setActiveTab('CAIXA')} className="col-span-12 md:col-span-8 bg-white rounded-xl shadow-sm border-l-4 border-purple-500 p-6 flex flex-col justify-center relative overflow-hidden cursor-pointer hover:-translate-y-1 hover:shadow-lg transition-all duration-200 group">
      ...
      <div className="bg-white p-6 rounded-xl shadow border">
          ...
          {teamFormMode === 'LIST' ? (
              <div className="space-y-3">
                  {teamUsers.map(u => {
                      const activeAdolescentsRole = getUserRoles(u).find(r => ADOLESCENTS_ROLES.some(ar => ar.role === r)) || (u.role as Role);
                      return (
                      <div key={u.id} className="p-4 border rounded-lg flex justify-between items-center hover:bg-gray-50 transition-colors">
                          <div className="flex items-center">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs text-gray-500">@{u.username} • <span className="text-purple-600 font-semibold">{ADOLESCENTS_ROLES.find(r => r.role === activeAdolescentsRole)?.label}</span></p>
                              </div>
                          </div>
