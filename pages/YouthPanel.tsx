...
export const YouthPanel: React.FC = () => {
  ...
  const handleSaveTemplate = async (asNew: boolean) => { if(!currentChurch) return; if(!templateName.trim()) { showFeedback('Digite um nome.', 'error'); return; } if(!backgroundUrl) { showFeedback('Carregue imagem.', 'error'); return; } setIsSavingSettings(true); if (asNew || !editingTemplateId) { const newT: CarnetTemplate = { id: '', churchId: currentChurch.id, name: templateName, backgroundUrl, backgroundStyle: bgStyle, layoutJson: layoutElements, isDefault: templates.length === 0 }; await addCarnetTemplate(newT); showFeedback('Salvo!'); } else { await updateCarnetTemplate(editingTemplateId, { name: templateName, backgroundUrl, backgroundStyle: bgStyle, layoutJson: layoutElements }); showFeedback('Atualizado!'); } await loadTemplates(); setIsSavingSettings(false); };
  ...
  const handleSetDefault = async (id: string) => { if(!currentChurch) return; await setDefaultTemplate(id, currentChurch.id); await loadTemplates(); showFeedback('Definido como padrão.'); };
  ...
  const renderReports = () => {
      // ... (Reports logic) ...
      const filteredReportTransactions = transactions.filter(t => { if (t.churchId !== currentChurch?.id) return false; if (t.category !== 'JOVENS') return false; if (t.status !== 'PAGO') return false; if (reportFilterType === 'MONTH') { const tDate = new Date(t.date + 'T12:00:00'); return (tDate.getMonth() + 1) === repMonth && tDate.getFullYear() === repYear; } else { return t.date >= appliedRepStart && t.date <= appliedRepEnd; } });
      const totalIn = filteredReportTransactions.filter(t => t.type === 'ENTRADA').reduce((acc, t) => acc + t.amount, 0); const totalOut = filteredReportTransactions.filter(t => t.type === 'SAIDA').reduce((acc, t) => acc + t.amount, 0); const repBalance = totalIn - totalOut; const inflows = filteredReportTransactions.filter(t => t.type === 'ENTRADA'); const outflows = filteredReportTransactions.filter(t => t.type === 'SAIDA'); const applyDateFilter = () => { setAppliedRepStart(repStartDate); setAppliedRepEnd(repEndDate); }; const generatePDFReport = () => { const doc = new jsPDF(); const periodText = reportFilterType === 'MONTH' ? `${repMonth.toString().padStart(2, '0')}/${repYear}` : `${new Date(appliedRepStart).toLocaleDateString()} a ${new Date(appliedRepEnd).toLocaleDateString()}`; doc.setFontSize(18); doc.text("Relatório de Jovens", 14, 20); doc.setFontSize(10); doc.text(`Período: ${periodText}`, 14, 26); doc.text(`Unidade: ${currentChurch?.name}`, 14, 32); if (reportViewMode === 'SUMMARY') { autoTable(doc, { startY: 40, head: [['Tipo', 'Valor']], body: [ ['Entradas', totalIn.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})], ['Saídas', totalOut.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})], ['Saldo Final', repBalance.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})] ] }); } else { autoTable(doc, { startY: 40, head: [['Data', 'Descrição', 'Tipo', 'Valor']], body: filteredReportTransactions.map(t => [ new Date(t.date).toLocaleDateString('pt-BR'), t.description, t.type, t.amount.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'}) ]) }); const finalY = (doc as any).lastAutoTable.finalY + 10; doc.text(`Total Entradas: ${totalIn.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}`, 14, finalY); doc.text(`Total Saídas: ${totalOut.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}`, 14, finalY + 6); doc.text(`Saldo: ${repBalance.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}`, 14, finalY + 12); } doc.save(`Relatorio_Jovens_${periodText.replace(/\//g, '-')}.pdf`); };
                ...
                {activeTab === 'CARNES' && (
                    <div className="space-y-6">
                        <div className="bg-white p-6 rounded-xl shadow border">
                            ...
                            <div className="bg-orange-50 p-3 rounded text-orange-800 text-sm mb-4">Selecione um modelo visual e preencha os dados abaixo para gerar o PDF.</div>
                            <div className="space-y-4">
                                <div><label className="block text-xs font-bold text-gray-500 mb-1">Selecione o Modelo de Impressão</label><select className="w-full p-2 border rounded bg-gray-50 text-sm" value={selectedGenTemplateId} onChange={e => setSelectedGenTemplateId(e.target.value)}><option value="">Selecione...</option>{templates.map(t => (<option key={t.id} value={t.id}>{t.name} {t.isDefault ? '(Padrão)' : ''}</option>))}</select>{templates.length === 0 && <p className="text-xs text-red-500 mt-1">Nenhum modelo cadastrado. Vá em "Config. Modelo" para criar.</p>}</div>
                                <div className="relative"><label className="block text-xs font-bold text-gray-500 mb-1">Membro Contribuinte</label><input type="text" className="w-full p-2 border rounded uppercase font-bold" placeholder="Digite o nome..." value={bookletSearchTerm} onChange={e => { setBookletSearchTerm(e.target.value.toUpperCase()); setBookletMemberId(''); }} />{bookletSearchTerm && !bookletMemberId && (<div className="absolute w-full bg-white shadow border mt-1 max-h-40 overflow-y-auto z-10">{filteredMembersForBooklet.map(m => (<div key={m.id} onClick={() => handleBookletMemberSelect(m)} className="p-2 hover:bg-gray-100 cursor-pointer text-xs font-bold">{m.name}</div>))}</div>)}{bookletMemberId && <div className="text-xs text-green-600 font-bold mt-1"><CheckCircle size={10} className="inline"/> {bookletSearchTerm}</div>}</div>
                                <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-500">Ano de Referência</label><input type="number" className="w-full p-2 border rounded" value={bookletYear} onChange={e => setBookletYear(parseInt(e.target.value))}/></div><div><label className="text-xs font-bold text-gray-500">Valor Mensal (R$)</label><input type="number" step="0.01" className="w-full p-2 border rounded" value={bookletAmount} onChange={e => setBookletAmount(e.target.value)}/></div></div>
                            </div>
                            <button onClick={generateBookletPDF} disabled={isGenerating} className="w-full py-3 mt-4 bg-brand-orange text-white rounded font-bold hover:bg-brand-red flex items-center justify-center disabled:opacity-70">{isGenerating ? <Loader className="animate-spin mr-2" size={18}/> : <Download className="mr-2" />} {isGenerating ? 'Gerando PDF...' : 'Gerar PDF do Carnê & Lançar'}</button>
                        </div>
                        
                        {/* LISTA DE HISTÓRICO DE CARNÊS (REIMPRESSÃO) */}
                        <div className="bg-white p-6 rounded-xl shadow border">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4"><h3 className="font-bold text-gray-700 flex items-center"><History className="mr-2" /> Histórico de Carnês Gerados</h3><div className="flex gap-2"><div className="flex items-center bg-gray-50 border rounded p-1"><Calendar size={14} className="text-gray-400 ml-2 mr-1"/><select value={historyMonth} onChange={(e) => setHistoryMonth(Number(e.target.value))} className="bg-transparent text-sm font-bold text-gray-700 outline-none cursor-pointer">{Array.from({length: 12}, (_, i) => (<option key={i+1} value={i+1}>{new Date(0, i).toLocaleString('pt-BR', {month: 'short'}).toUpperCase()}</option>))}</select><select value={historyYear} onChange={(e) => setHistoryYear(Number(e.target.value))} className="bg-transparent text-sm font-bold text-gray-700 outline-none ml-2 border-l pl-2 cursor-pointer"><option value={2023}>2023</option><option value={2024}>2024</option><option value={2025}>2025</option></select></div></div></div><div className="relative mb-4"><input type="text" className="w-full p-2 pl-8 border rounded text-sm uppercase" placeholder="Filtrar por nome..." value={historySearch} onChange={(e) => setHistorySearch(e.target.value.toUpperCase())} /><Search className="absolute left-2.5 top-2.5 text-gray-400" size={16}/></div><div className="overflow-x-auto max-h-80 scrollbar-thin scrollbar-thumb-gray-300"><table className="w-full text-sm text-left"><thead className="bg-gray-50 text-gray-500 font-bold uppercase text-xs sticky top-0"><tr><th className="px-3 py-2">Vencimento</th><th className="px-3 py-2">Membro / Descrição</th><th className="px-3 py-2 text-right">Valor</th><th className="px-3 py-2 text-center">Ações</th></tr></thead><tbody>{historyTransactions.filter(t => t.description.includes('CARNÊ')).map(t => { const tDate = new Date(t.date); const memberName = members.find(m => m.id === t.memberId)?.name || 'N/A'; return (<tr key={t.id} className="border-b last:border-0 hover:bg-gray-50"><td className="px-3 py-2 whitespace-nowrap text-gray-600">{tDate.getDate().toString().padStart(2, '0')}/{ (tDate.getMonth() + 1).toString().padStart(2, '0') }</td><td className="px-3 py-2"><div className="font-bold text-gray-800 text-xs">{memberName}</div><div className="text-[10px] text-gray-500">{t.description}</div></td><td className="px-3 py-2 text-right font-bold text-gray-700">R$ {t.amount.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</td><td className="px-3 py-2 text-center"><div className="flex items-center justify-center gap-2"><button onClick={() => handleReprintCarnet(t)} className="text-blue-500 hover:text-blue-700 p-1 rounded hover:bg-blue-50 transition-colors" title="Baixar Carnê Completo"><Download size={14}/></button><button onClick={() => handleDeleteTransaction(t.id)} className="text-gray-400 hover:text-red-600 p-1 rounded hover:bg-red-50 transition-colors" title="Excluir Lançamento"><Trash2 size={14}/></button></div></td></tr>); })} {historyTransactions.filter(t => t.description.includes('CARNÊ')).length === 0 && (<tr><td colSpan={4} className="text-center py-6 text-gray-400 text-xs">Nenhum registro encontrado para este período.</td></tr>)}</tbody></table></div>
                        </div>
                    </div>
                )}
