...
export const YouthPanel: React.FC = () => {
  ...
  const canManageTeam = user && ['SUPER_ADM', 'PRESIDENTE', 'VICE_PRESIDENTE', 'DIRIGENTE', 'SECRETARIO', 'TESOUREIRO', 'LIDER_JOVENS', 'TESOUREIRO_JOVENS'].includes(user.role);
  const canAddToTeam = user && ['SUPER_ADM', 'PRESIDENTE', 'VICE_PRESIDENTE', 'DIRIGENTE', 'SECRETARIO', 'LIDER_JOVENS'].includes(user.role);
  ...
  const [showExpCatPopover, setShowExpCatPopover] = useState(false);
  const [expCat, setExpCat] = useState('');
  const expCatPopoverRef = useRef<HTMLDivElement>(null);
  ...
  const loadCarnetHistory = async () => {
      if (!currentChurch) return;
      const records = await getCarnetHistory(currentChurch.id, 'JOVENS');
      setCarnetHistoryRecords(records);
  };

  useEffect(() => {
    if (!showExpCatPopover) return;
    const handler = (e: MouseEvent) => {
      if (expCatPopoverRef.current && !expCatPopoverRef.current.contains(e.target as Node)) {
        setShowExpCatPopover(false);
      }
    };
  ...
  const handleReemitirCarnetRecord = async (r: CarnetHistoryRecord) => {
      ...
      const templateToUse = templates.find(temp => temp.name === r.templateName) ||
                            templates.find(temp => temp.isDefault) ||
                            templates[0];

      if (!templateToUse) {
          showFeedback('Nenhum modelo cadastrado para reemissão.', 'error');
          return;
      }
      ...
      try {
          ...
          doc.save(`REEMISSAO_CARNE_JOVENS_${r.memberName.replace(/\s+/g, '_')}_${r.year}.pdf`);
  ...
  // Team Handlers
  const teamUsers = users.filter(u => u.churchId === currentChurch?.id && getUserRoles(u).some(r => YOUTH_ROLES.some(yr => yr.role === r)));
  ...
  const handleSaveTeamMember = async (e: React.FormEvent) => {
    ...
    if (editingUserId) {
      ...
      if (existingUser) {
        const otherRoles = getUserRoles(existingUser).filter(r => !YOUTH_ROLES.some(yr => yr.role === r));
        ...
        showFeedback('Membro atualizado com sucesso!');
      }
    } else {
      // Verifica se já existe um usuário com esse username ou CPF
      ...
      if (existingUser) {
        // Vincula o novo cargo mantendo os anteriores
        ...
        showFeedback('Usuário já existente: novo cargo adicionado com sucesso!');
      } else {
        const payload: User = {
          id: '',
          name: teamFormData.name.toUpperCase(),
        ...
        if (res.success) showFeedback('Adicionado à equipe!');
        else showFeedback(res.error || 'Erro ao adicionar', 'error');
      }
    }
  ...
  const _unusedRenderVisualEditor = () => ( 
  ...
  const renderDashboard = () => {
      ...
      const monthlyTransactions = transactions.filter(t => t.churchId === currentChurch?.id && t.category === 'JOVENS' && t.status === 'PAGO' && !isCarnet(t) && new Date(t.date).getMonth() + 1 === dashMonth && new Date(t.date).getFullYear() === dashYear);
      ...
      const cumulativeTransactions = transactions.filter(t => t.churchId === currentChurch?.id && t.category === 'JOVENS' && t.status === 'PAGO' && !isCarnet(t) && t.date <= new Date(dashYear, dashMonth, 0).toISOString().split('T')[0]);
                                        ...
                                        <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-gray-500 mt-0.5">
                                                    Serão geradas <span className="font-bold">12 parcelas</span> (Jan–Dez de {new Date().getFullYear()}) para controle do membro (não gera transações financeiras).
                                                </p>
                                                    ...
                                                    <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50">
                                                        ...
                                                        <td className="px-3 py-2 text-center whitespace-nowrap">
                                                            <button 
                                                                ...
                                                                className="inline-flex items-center gap-1.5 text-xs font-bold text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 px-2.5 py-1.5 rounded-lg transition-all mr-2 shadow-sm active:scale-95 disabled:opacity-50" 
                                                            ...
                                                            <button onClick={async () => { await deleteCarnetHistory(r.id); await loadCarnetHistory(); }} className="text-gray-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 transition-colors" title="Remover do histórico"><Trash2 size={14}/></button>
                                                        </td>
                                                    </tr>
                                                );
                                            })}
