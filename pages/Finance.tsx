...
export const Finance: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    showConfirm('Excluir', 'Confirmar exclusão?', () => deleteTransaction(id), 'danger');
  };
       ...
       <form onSubmit={handleSubmit} className="space-y-3 md:space-y-6">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {[ { val: 'DIZIMO', label: 'DÍZIMO' }, { val: 'MISSOES', label: 'MISSÕES' }, { val: 'OFERTA', label: 'OFERTA' } ].map((item) => (
                  <button type="button" key={item.val} onClick={() => { setCategory(item.val as TransactionCategory); if (!editingTransactionId) clearMemberSelection(); }}
                    className={`flex-1 min-w-[80px] px-2 py-2 rounded-lg border font-bold text-xs transition-colors ${category === item.val ? 'bg-brand-orange text-white border-brand-orange' : 'border-gray-300 text-gray-700'}`}
                  >
                    {item.label}
                  </button>
                ))}
          ...
          {activeTab === 'ENTRADA' && (category === 'DIZIMO' || category === 'MISSOES') && (
            <div className="bg-orange-50 p-2 rounded-lg border border-orange-100">
              ...
              <div className="relative">
                <input type="text" placeholder="NOME..." className="w-full p-2 pl-8 border rounded-lg uppercase text-sm" value={searchTerm}
                  onChange={e => { setSearchTerm(e.target.value.toUpperCase()); if(selectedMemberId) setSelectedMemberId(''); }}
                />
                ...
                <div className="mt-1 max-h-40 overflow-y-auto bg-white border rounded shadow-lg z-10">
                  {filteredMembers.length > 0 ? (
                      filteredMembers.map(m => (
                        <div key={m.id} onClick={() => handleMemberSelect(m)} className="p-2 hover:bg-orange-100 cursor-pointer border-b text-xs">
                          <span className="font-bold">{m.name}</span>
                        </div>
                      ))
                  ) : <div className="p-2 text-gray-500 text-xs text-center">Não encontrado.</div>}
                </div>
              )}
              
              {selectedMemberId && (
                  <div className="mt-1 text-green-700 text-xs font-bold flex items-center bg-green-50 p-1 rounded">
                      <CheckCircle size={14} className="mr-1"/> {searchTerm}
                  </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'ENTRADA' && category === 'OFERTA' && (
             <div>
               ...
               <input type="text" required placeholder="EX: CULTO DOMINGO" className="mt-1 w-full p-2 border rounded-lg uppercase text-sm" value={description} onChange={e => setDescription(e.target.value.toUpperCase())} />
             </div>
          )}

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700">Valor (R$)</label>
              <input type="number" step="0.01" required className="mt-1 w-full p-2 border rounded-lg font-bold" value={amount} onChange={e => setAmount(e.target.value)} />
            </div>
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700">Data</label>
              <input type="date" required className="mt-1 w-full p-2 border rounded-lg" value={date} onChange={e => setDate(e.target.value)} />
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-2">
             ...
             <button type="submit" disabled={isSubmitting} className={`flex-[2] py-2 rounded-lg font-bold text-white flex justify-center items-center text-sm ${activeTab === 'ENTRADA' ? 'bg-green-600' : 'bg-brand-red'}`}>
                {isSubmitting ? <Loader className="animate-spin" size={16}/> : 'Salvar'}
             </button>
          </div>
       </form>
    </div>
  ...
  const renderList = () => (
    <div className="space-y-3">
      ...
      <div className="bg-white p-2 rounded-lg shadow-sm border border-gray-100 flex flex-wrap gap-2 items-center justify-between">
          <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-bold border ${showFilters || filterType !== 'TODOS' ? 'bg-brand-black text-white' : 'bg-gray-50 text-gray-600'}`}>
             <Filter size={14} /> <span>{filterType === 'TODOS' ? 'Filtrar' : filterType}</span>
          </button>

          <div className="flex items-center bg-gray-50 rounded border border-gray-200 p-0.5">
              <select value={filterMonth} onChange={e => setFilterMonth(parseInt(e.target.value))} className="bg-transparent border-none text-xs font-bold text-gray-700 py-1 pl-1 pr-6 cursor-pointer">
          ...
          {showFilters && (
             <div className="w-full flex gap-2 overflow-x-auto pb-1 mt-1 border-t pt-2">
                 {[{ id: 'TODOS', label: 'Tudo' }, { id: 'DIZIMO', label: 'Dízimo' }, { id: 'MISSOES', label: 'Missões' }, { id: 'OFERTA', label: 'Oferta' }, { id: 'SAIDA', label: 'Saída' }].map(f => (
                     <button key={f.id} onClick={() => { setFilterType(f.id as any); setShowFilters(false); }} className={`px-2 py-1 rounded text-[10px] font-bold border whitespace-nowrap ${filterType === f.id ? 'bg-brand-orange text-white' : 'bg-white'}`}>{f.label}</button>
                 ))}
             </div>
          )}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-2 py-2 text-left text-[9px] font-bold text-gray-500 uppercase">Data</th>
                <th className="px-2 py-2 text-left text-[9px] font-bold text-gray-500 uppercase">Desc</th>
                <th className="hidden md:table-cell px-2 py-2 text-left text-[9px] font-bold text-gray-500 uppercase">Cat</th>
                <th className="px-2 py-2 text-right text-[9px] font-bold text-gray-500 uppercase">Valor</th>
                <th className="px-1 py-2 text-right text-[9px] font-bold text-gray-500 uppercase">.</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {churchTransactions.map(t => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="hidden md:table-cell px-2 py-2 text-xs">
                    <span className={`px-1 rounded text-[10px] font-bold ${t.type === 'ENTRADA' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{formatCategoryName(t.category, t.type)}</span>
                  </td>
                  <td className={`px-2 py-2 text-[10px] font-bold text-right whitespace-nowrap ${t.type === 'ENTRADA' ? 'text-green-600' : 'text-brand-red'}`}>
                  ...
                  <td className="px-1 py-2 text-right whitespace-nowrap">
                      {canEdit && (
                          <div className="flex justify-end gap-2">
                              <button onClick={() => handleEdit(t)} className="text-gray-400 hover:text-brand-orange"><Edit2 size={14}/></button>
                              <button onClick={() => handleDelete(t.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={14}/></button>
                          </div>
                      )}
                  </td>
                </tr>
