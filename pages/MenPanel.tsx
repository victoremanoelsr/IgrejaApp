...
export const MenPanel: React.FC = () => {
  ...
  const teamUsers = users.filter(u => u.churchId === currentChurch?.id && getUserRoles(u).some(r => MEN_ROLES.some(mr => mr.role === r)));
      ...
      <div className="bg-white p-6 rounded-xl shadow border">
          ...
          {teamFormMode === 'LIST' ? (
              <div className="space-y-3">
                  {teamUsers.map(u => {
                      const activeMenRole = getUserRoles(u).find(r => MEN_ROLES.some(mr => mr.role === r)) || (u.role as Role);
                      return (
                      <div key={u.id} className="p-4 border rounded-lg flex justify-between items-center hover:bg-gray-50 transition-colors">
                          <div className="flex items-center">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs text-gray-500">@{u.username} • <span className="text-blue-600 font-semibold">{MEN_ROLES.find(r => r.role === activeMenRole)?.label}</span></p>
                              </div>
                          </div>
